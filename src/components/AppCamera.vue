<template>
  <div>
    ...
    <v-dialog persistent data-app="true" v-model="dialog" :max-width="options.width" @keydown.esc="cancel" v-bind:style="{ zIndex: options.zIndex }">
      <v-card>
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12>
                  <v-text-field type="password" v-model="pin" label="PIN" hint="Please enter your PIN." required ></v-text-field>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card-text>
  ...
  methods: {
    ...
    doLogin () {
      ...
      chain.doLogin(this.pin, this.qrResult, (err) => {
        ...
        if (!err) {
          this.$root.$emit('error_on', 'Login data sent successfully.', 'green')
          router.push('/')
        } else {
          if ( err == 'authError') {
      ...
      chain.doExecSelection(this.pin, this.qrResult, (err) => {        
        ...
        if (!err) {
          if (this.pinCase === 'decrypt') {
            this.$root.$emit('error_on', 'Decrypted data successfully.', 'green')
    ...
    onDecode (qrResult) {
      ...
      if (qrResult.indexOf('/login') > 0) {
        ...
        this.pinCase = 'share'
        this.open('Document Share Request', 'You are about to share a document. Are you sure?')
          .then((resolved) => {            
            if (resolved) {
              if (chain.pinned()) {
                this.showPinDialog = true
        ...
        this.pinCase = 'sign'
        this.open('File Sign Request', 'You are about to sign a file. Are you sure?')
          .then((resolved) => {            
            if (resolved) {
              if (chain.pinned()) {
                this.showPinDialog = true
